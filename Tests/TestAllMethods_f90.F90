...
subroutine TestAllMethods_f90()  BIND(C, NAME='TestAllMethods_f90')
  ...
  ! --------------------------------------------------------------------------
	...
	status = bmif%LoadDatabase("phreeqc.dat")
	write(*,*) "LoadDatabase"
	!
	! Set properties
	!
	...
	status = bmif%SetComponentH2O(0)
	write(*,*) "SetComponentH2O "
	!-------
	status = RM_SetSpeciesSaveOn(id, 1)
	status = bmif%SetSpeciesSaveOn(1)
	...
	status =bmif%SetUnitsPPassemblage(1)
	...
	status = bmif%SetRepresentativeVolume(DoubleVector)
	...
	nchem = bmif%GetChemistryCellCount()
	write(*,*) "GetChemistryCellCount "
	!-------
	ncomps = RM_GetComponentCount(id)
	ncomps = bmif%GetComponentCount()
	...
	status = bmif%GetComponents(StringVector)
	write(*,*) "GetComponents)" 
	! Species info
	nspecies = RM_GetSpeciesCount(id)
	nspecies = bmif%GetSpeciesCount()
	write(*,*) "GetSpeciesCount "
	!-------
	status = RM_GetSpeciesNames(id, StringVector)
	status = bmif%GetSpeciesNames(StringVector)
	...
	status = bmif%InitialPhreeqc2Module(IntVector2)
	...
	status = bmif%InitialPhreeqc2Module(v1, v2, f2)
	...
	status = bmif%InitialPhreeqcCell2Module(1, cells, size(cells))
	...
	status = bmif%InitialPhreeqc2Concentrations(bc2, size(u1), u1, u2, f1)
	...
	status = bmif%GetPorosity(DoubleVector)
	...
	status = bmif%SetSaturationUser(DoubleVector)
	...
	status = bmif%GetSolutionVolume(DoubleVector)
	...
	status = bmif%GetViscosity(DoubleVector)
	...
	n = bmif%GetNthSelectedOutputUserNumber(1)
    ...
    n = bmif%GetSelectedOutputColumnCount()
    allocate(DoubleVector2(nxyz, n))
	status = RM_GetSelectedOutput(id, DoubleVector2)
	status = bmif%GetSelectedOutput(DoubleVector2)
	...
	status = bmif%GetSelectedOutputHeadings(StringVector)
	write(*,*) "GetSelectedOutputHeadings "
	!-------
	!b = RM_GetSelectedOutputOn(id)
	!b = bmif%GetSelectedOutputOn()
	...
	n = bmif%GetSelectedOutputRowCount()
