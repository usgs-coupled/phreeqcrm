...
INTEGER FUNCTION RM_GetSurfaceDiffuseLayerArea(id, surf, area) 
	USE ISO_C_BINDING  
    IMPLICIT NONE
    INTERFACE
        INTEGER(KIND=C_INT) FUNCTION RMF_GetSurfaceDiffuseLayerArea(id, surf, area) &
			BIND(C, NAME='RMF_GetSurfaceDiffuseLayerArea')   
			USE ISO_C_BINDING
            IMPLICIT NONE
            INTEGER(KIND=C_INT), INTENT(in) :: id
            CHARACTER(KIND=C_CHAR), INTENT(in) :: surf(*)
            REAL(KIND=C_DOUBLE), INTENT(out) :: area(*)
        END FUNCTION RMF_GetSurfaceDiffuseLayerArea 
	END INTERFACE
    INTEGER, INTENT(in) :: id
    CHARACTER(len=*), INTENT(in) :: surf
    DOUBLE PRECISION, INTENT(out), DIMENSION(:) :: area
	if (rmf_debug) call Chk_GetSurfaceDiffuseLayerArea(id, area)
    RM_GetSurfaceDiffuseLayerArea = RMF_GetSurfaceDiffuseLayerArea(id, trim(surf)//C_NULL_CHAR, area)
END FUNCTION RM_GetSurfaceDiffuseLayerArea 

SUBROUTINE Chk_GetSurfaceDiffuseLayerArea(id, area)
    IMPLICIT NONE
    INTEGER, INTENT(in) :: id
    DOUBLE PRECISION, INTENT(in), DIMENSION(:) :: area
    INTEGER :: errors
    errors = 0
    errors = errors + Chk_Double1D(id, area, rmf_nxyz, "diffuse layer area", "Chk_GetSurfaceDiffuseLayerArea")
    if (errors .gt. 0) then
        errors = RM_Abort(id, -3, "Invalid argument in Chk_GetSurfaceDiffuseLayerArea")
    endif
END SUBROUTINE Chk_GetSurfaceDiffuseLayerArea
...
INTEGER FUNCTION RM_GetSurfaceDiffuseLayerName(id, num, surf_name)
	USE ISO_C_BINDING
    IMPLICIT NONE
    INTERFACE
        INTEGER(KIND=C_INT) FUNCTION RMF_GetSurfaceDiffuseLayerName(id, num, surf_name, l) &
			BIND(C, NAME='RMF_GetSurfaceDiffuseLayerName')
			USE ISO_C_BINDING
            IMPLICIT NONE
            INTEGER(KIND=C_INT), INTENT(in) :: id, num, l
            CHARACTER(KIND=C_CHAR), INTENT(out) :: surf_name(*)
...
!> @param thickness     Array to receive the diffuse-layer thicknesses.
!> Dimension of the array must be @a nxyz,
!> where @a nxyz is the number of grid cells (@ref RM_GetGridCellCount).
...
!> do i = 1, count_surface
!>   status = RM_GetSurfaceDiffuseLayerName(id, i, surfaces(i))
!>   status = RM_GetSurfaceDiffuseLayerArea(id, surfaces(i), dl_thicknesses)
!> enddo
!> </PRE>
!> </CODE>
!> @endhtmlonly
...
INTEGER FUNCTION RM_GetSurfaceDiffuseLayerThickness(id, surf, Thickness) 
	USE ISO_C_BINDING  
    IMPLICIT NONE
    INTERFACE
        INTEGER(KIND=C_INT) FUNCTION RMF_GetSurfaceDiffuseLayerThickness(id, surf, Thickness) &
			BIND(C, NAME='RMF_GetSurfaceDiffuseLayerThickness')   
			USE ISO_C_BINDING
            IMPLICIT NONE
            INTEGER(KIND=C_INT), INTENT(in) :: id
            CHARACTER(KIND=C_CHAR), INTENT(in) :: surf(*)
            REAL(KIND=C_DOUBLE), INTENT(out) :: Thickness(*)
        END FUNCTION RMF_GetSurfaceDiffuseLayerThickness 
	END INTERFACE
    INTEGER, INTENT(in) :: id
    CHARACTER(len=*), INTENT(in) :: surf
    DOUBLE PRECISION, INTENT(out), DIMENSION(:) :: Thickness
	if (rmf_debug) call Chk_GetSurfaceDiffuseLayerThickness(id, Thickness)
    RM_GetSurfaceDiffuseLayerThickness = RMF_GetSurfaceDiffuseLayerThickness(id, trim(surf)//C_NULL_CHAR, Thickness)
END FUNCTION RM_GetSurfaceDiffuseLayerThickness

SUBROUTINE Chk_GetSurfaceDiffuseLayerThickness(id, Thickness)
    IMPLICIT NONE
    INTEGER, INTENT(in) :: id
    DOUBLE PRECISION, INTENT(in), DIMENSION(:) :: Thickness
    INTEGER :: errors
    errors = 0
    errors = errors + Chk_Double1D(id, Thickness, rmf_nxyz, "diffuse layer thickness", "Chk_GetSurfaceDiffuseLayerThickness")
    if (errors .gt. 0) then
        errors = RM_Abort(id, -3, "Invalid argument in Chk_GetSurfaceDiffuseLayerThickness")
    endif
END SUBROUTINE Chk_GetSurfaceDiffuseLayerThickness
...
INTEGER FUNCTION RM_SetSurfaceDiffuseLayerConcentrations(id, surf, dl_species_conc)
	USE ISO_C_BINDING
    IMPLICIT NONE
    INTERFACE
        INTEGER(KIND=C_INT) FUNCTION RMF_SetSurfaceDiffuseLayerConcentrations(id, surf, dl_species_conc) &
			BIND(C, NAME='RMF_SetSurfaceDiffuseLayerConcentrations')
			USE ISO_C_BINDING
            IMPLICIT NONE
            INTEGER(KIND=C_INT), INTENT(in) :: id
            CHARACTER(KIND=C_CHAR), INTENT(in) :: surf(*)
            REAL(KIND=C_DOUBLE), INTENT(in) :: dl_species_conc(*)
        END FUNCTION RMF_SetSurfaceDiffuseLayerConcentrations
	END INTERFACE
    INTEGER, INTENT(in) :: id
    CHARACTER(len=*), INTENT(in) :: surf
    DOUBLE PRECISION, DIMENSION(:,:), INTENT(in) :: dl_species_conc
	if (rmf_debug) call Chk_SetSurfaceDiffuseLayerConcentrations(id, dl_species_conc)
    RM_SetSurfaceDiffuseLayerConcentrations = RMF_SetSurfaceDiffuseLayerConcentrations(id, surf, dl_species_conc)
END FUNCTION RM_SetSurfaceDiffuseLayerConcentrations  

SUBROUTINE Chk_SetSurfaceDiffuseLayerConcentrations(id, dl_species_conc)
    IMPLICIT NONE
    INTEGER, INTENT(in) :: id
    DOUBLE PRECISION, DIMENSION(:,:), INTENT(in) :: dl_species_conc
    INTEGER :: errors, nspecies
    nspecies = RM_GetSpeciesCount(id)
    errors = 0
    errors = errors + Chk_Double2D(id, dl_species_conc, rmf_nxyz, nspecies, "species_conc", "RM_SetSurfaceDiffuseLayerConcentrations")
    if (errors .gt. 0) then
        errors = RM_Abort(id, -3, "Invalid argument in RM_SetSurfaceDiffuseLayerConcentrations")
    endif
END SUBROUTINE Chk_SetSurfaceDiffuseLayerConcentrations
