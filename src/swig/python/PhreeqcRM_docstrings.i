...
product of porosity (:meth:`SetPorosity`), the saturation (:meth:`SetSaturationUser`),
...
SetSaturationUser`), and representative volume (:meth:`SetRepresentativeVolume`). To
...
/kgs is converted to mol/kgs, multiplied by density (:meth:`SetDensityUser`) and 
multiplied by the solution volume.
...
saturation (:meth:`SetSaturationUser`), and representative volume 
(:meth:`SetRepresentativeVolume`), and the mass of solution is volume times 
density as defined by :meth:`SetDensityUser`. Which option is used is determined by 
...
(2) the specified density (:meth:`SetDensityUser`) and solution volume are 
determined by the product of saturation (:meth:`SetSaturationUser`), porosity 
...
Args:
	tf (Boolean): True indicates that the solution density and volume as 
		...
		indicates that the solution density set by :meth:`SetDensityUser` and the volume
		determined by the product of  :meth:`SetSaturationUser`, :meth:`SetPorosity`, 
...
Args:
	name (str): Name of the variable to retrieve size.
...
SetConcentrations(float list, numpy.ndarray, or tuple
				  : c);
SetCurrentSelectedOutputUserNumber(int n_user);
SetDensityUser(float list, numpy.ndarray, or tuple
		   : density);
SetDumpFileName(string dump_name);
SetErrorHandlerMode(int mode);
SetErrorOn(Boolean tf);
...
SetRepresentativeVolume(float list, numpy.ndarray, or tuple
						: rv);
SetSaturationUser(float list, numpy.ndarray, or tuple
			  : sat);
SetScreenOn(Boolean tf);
SetSelectedOutputOn(Boolean tf);
SetSpeciesSaveOn(Boolean save_on);
...
'Saturation', :meth:`SetSaturationUser`;
