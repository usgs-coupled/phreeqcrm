...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_GetSpeciesLogGammas(int id, double * log_gamma)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	if (Reaction_module_ptr)
	{
		if (log_gamma != NULL)
		{
			...
			std::vector<double> log_gamma_vector;
			return_value = Reaction_module_ptr->GetSpeciesLogGammas(log_gamma_vector);
			memcpy(log_gamma, &log_gamma_vector.front(), log_gamma_vector.size()*sizeof(double));
			return IRM_OK;
		}
		return IRM_INVALIDARG;
	}
...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_GetSurfaceDiffuseLayerArea(int id, char *surf, double * areas)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	IRM_RESULT return_value = IRM_OK;
...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_GetSurfaceDiffuseLayerConcentrations(int id, char *surf, double * c)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	IRM_RESULT return_value = IRM_OK;
...
/* ---------------------------------------------------------------------- */
int RM_GetSurfaceDiffuseLayerCount(int id)
/* ---------------------------------------------------------------------- */
{
	// Returns the number of components 
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_GetSurfaceDiffuseLayerName(int id, int num, char *surf_name, int l1)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	if (Reaction_module_ptr)
...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_GetSurfaceDiffuseLayerThickness(int id, char *surf, double * thickness)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	IRM_RESULT return_value = IRM_OK;
...
/* ---------------------------------------------------------------------- */
IRM_RESULT
RM_SetSurfaceDiffuseLayerConcentrations(int id, char *surf, double * c)
/* ---------------------------------------------------------------------- */
{
	PhreeqcRM * Reaction_module_ptr = PhreeqcRM::GetInstance(id);
	IRM_RESULT return_value = IRM_OK;
