...
<CODE>
<PRE>
status = WriteYAMLDoc(id, "AdvectBMI_C.yaml");
status = YAMLClear(id);
...
@par
CreateMapping
provides a mapping from grid cells in the user's model to reaction cells for which chemistry
needs to be run.
...
@param cell_numbers       A vector of grid-cell numbers (user's grid
...
initialize a PhreeqcRM instance.
@param id     The instance id returned from @ref CreateYAMLPhreeqcRM.
@param tf               1 (default), excess H, excess O, and water are included in the component list;
0, total H and O are included in the component list.
@retval IRM_RESULT   Zero indicates success, negative indicates failure.
@par
...
The default setting (1) is to include water, excess H, and excess O as components.
A setting of 0 will include total H and total O as components.
...
for(i = 0; i < nxyz; i++) density[i] = 1.0d0;
...
0 (default), all solids and gases are reactive regardless of saturation.
@retval IRM_RESULT   Zero indicates success, negative indicates failure.
@par
...
for(i = 0; i < nxyz; i++) pressure[i] = 2.0d0;
status = YAMLSetPorosity(id, pressure, nxyz);
</PRE>
</CODE>
@endhtmlonly
*/
